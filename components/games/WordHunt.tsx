...
function generateGrid(difficulty: Difficulty): { grid: string[][]; possibleWords: string[] } {
  ...
  for (let attempt = 0; attempt < 200; attempt++) {
    ...
    for (const word of candidates) {
      ...
      const minRow = Math.max(0, -dir[0]) * (word.length - 1);
      const maxRow = size - 1 - Math.max(0, dir[0]) * (word.length - 1);
      const minCol = Math.max(0, -dir[1]) * (word.length - 1);
      const maxCol = size - 1 - Math.max(0, dir[1]) * (word.length - 1);
